...
PicoLM prints a full memory report to stderr, including tracked allocations and OS-level process memory:

```
Memory breakdown (tracked total: ... MB):
  runtime state: ...
  KV cache: ...
  tokenizer: ...
  prompt token buffer: ...
  stdin prompt buffer: ...
model mmap (virtual): ...
Process memory (OS-level):
  RSS (resident): ...
  VMS (virtual): ...
```

Use **tracked total** for PicoLM-owned allocations, and **RSS** for physical RAM usage. `model mmap (virtual)` is reported separately because it is virtual address space, not part of tracked allocations.
